...
- The live theme is **GitHub-connected**: `moonridge-shopify-theme/main` (theme ID `gid://shopify/OnlineStoreTheme/184590074160`), synced to `github.com/rwakefi/moonridge-shopify-theme` branch `main`. The repo is publicly clonable, so a plain `git clone` works from anywhere for reading.
- **Cowork can write to the repo — use the GitHub connector, not git.** (Corrected 2026-09-10; earlier notes here called Cowork read-only, which was only ever true of `git push`.) The **GitHub connector (MCP) has write access**: it creates branches, creates/updates files, and opens PRs. That is how PR #127 was raised. What does *not* work from the Cowork sandbox: `git push` (the git proxy refuses to inject a credential because the repo isn't in the session's authorized repository set — 403), direct `curl` to `api.github.com` with the sandbox token (403 the same way), the `gh` CLI (not installed), and an authenticated Shopify CLI. The proxy's own error names the fix for `git push` — add the repo to the session's sources — but that is untested so far.
- **Verify every connector write.** Clone the repo, make the edit locally with real tooling so the diff is reviewable and testable, then upload through the connector and check that the blob SHA the API returns matches `git hash-object <file>` locally. Matching hashes prove the upload is byte-identical to what you tested — the connector takes whole-file content, so this is the guard against a silent transcription error on a live-theme file.
- **An agent stages, a human merges.** Cowork's safety classifier blocks `merge_pull_request`, so an agent can open and populate a PR but cannot merge it. **Merging to `main` is publishing** — Shopify syncs within about a minute. Treat a merge as a live store change and get Zack's explicit go-ahead first (verified Aug 2026, PR #81 was live ~60s after push).
- **Cloud Agents are different:** they have git push credentials and a read-only `gh`.
